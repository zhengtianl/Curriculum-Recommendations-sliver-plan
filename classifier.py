...
# =========================================================================================
def read_data(cfg):
    train = pd.read_csv(f'/content/exp/train_top{CFG.top_n}_fold2_cv_with_groundtruth_final_filter.csv')#.head(1000)
    train['title1'].fillna("Title does not exist", inplace = True)
    train['title2'].fillna("Title does not exist", inplace = True)
    correlations = pd.read_csv('/content/learning-equality-curriculum-recommendations/correlations.csv')
    ...
    for epoch in range(cfg.epochs):
        ...
        if score > best_score:
            ...
            torch.save(
                ...
                f"/content/exp/xlm_large_fold{fold}_{cfg.seed}_content_top20.pth"
                )
...
train_and_evaluate_one_fold(train, correlations, 2, CFG)
